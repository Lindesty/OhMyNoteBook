...
```
扩展：上述代码，还需要编写一个不写`setup`的`script`标签，去指定组件名字，比较麻烦。

> **Vue 3.3+ 推荐方式**：使用 `defineOptions` 宏（编译器宏，无需导入）
> ```vue
> <script setup lang="ts">
...
**旧方式（不推荐）**：使用 `vite-plugin-vue-setup-extend` 插件（已过时）
...
<script lang="ts" setup name="Person">
  ...
  // 注意：defineExpose是编译器宏，不需要从vue导入
...
>
>   // 第三种写法：接收+限制类型+指定默认值+限制必要性（Vue 3.4及以下）
> let props = withDefaults(defineProps<{list?:Persons}>(),{
>      list:()=>[{id:'asdasg01',name:'小猪佩奇',age:18}]
>   })
>    console.log(props)
>
>   // Vue 3.5+ 新语法：响应式解构 + 原生默认值
>   // const { list = [{id:'asdasg01',name:'小猪佩奇',age:18}] } = defineProps<{list?:Persons}>()
>   </script>
>   ```

## 3.12.1.【Vue 3.3+ 新特性：简洁的 emit 类型声明】

`defineEmits` 在 Vue 3.3+ 支持更简洁的命名元组语法：
...
## 3.12.2.【Vue 3.5+ 新特性：响应式 Props 解构】

Vue 3.5+ 支持响应式解构 props，解构后的变量保持响应性：
...
// Vue 3.5+ 响应式解构 + 原生默认值语法
const { title, count = 0, items = [] } = defineProps<Props>()

// watchEffect 会随 props 变化重新执行（Vue 3.5+）
watchEffect(() => {
  console.log(title)  // 响应式跟踪
})
</script>
```

> **对比**：Vue 3.4 及以下版本使用 `withDefaults`：
> ```ts
...
6. **Vue 3.4+ 新特性：`defineModel` 宏**（推荐使用）

   > `defineModel` 是 Vue 3.4 新增的编译器宏，大幅简化了 v-model 双向绑定的实现。

   使用 `defineModel` 重写上面的 `AtguiguInput` 组件：
   ...
   <script setup lang="ts" name="AtguiguInput">
     // defineModel 自动声明 prop 和 update 事件
     const model = defineModel()

     // 直接修改 model.value 会自动触发 update:modelValue 事件
   ...
   **带参数的 v-model**：
   ...
   ```vue
   <script setup lang="ts" name="AtguiguInput">
     // 声明 abc prop，配合 v-model:abc 使用
     const abc = defineModel('abc')

     // 声明 xyz prop，配合 v-model:xyz 使用
     const xyz = defineModel('xyz')
   </script>
   ```
   ...
   **带选项的 defineModel**：
   ...
   > **注意**：`defineModel` 是编译器宏，无需从 vue 导入。
...
## 8.5.【Vue 3.3+ 新特性：defineOptions】

在 `<script setup>` 中，可以使用 `defineOptions` 宏定义组件选项：
...
> **说明**：`defineOptions` 是 Vue 3.3 新增的编译器宏，用于在 `<script setup>` 中定义组件级别的选项，如组件名称、`inheritAttrs` 等。无需从 vue 导入。

## 8.6.【Vue 3.4+ 新特性：defineModel】

详见 [6.4.【v-model】](#64v-model) 章节，`defineModel` 大幅简化了双向绑定的实现。

---

# 9. Vue 3.3~3.5 版本新特性汇总

> 本节汇总 Vue 3.3 至 3.5 版本的主要新特性，帮助开发者了解最新变化。

## 9.1.【Vue 3.3 新特性】

- **defineEmits 简洁语法**：支持命名元组语法声明 emit 类型
- **defineOptions 宏**：在 `<script setup>` 中定义组件选项
- **更好的 TypeScript 支持**：类型导入简化、泛型组件改进

## 9.2.【Vue 3.4 新特性】

- **defineModel 宏**：简化 v-model 双向绑定的实现
- **v-bind 同名简写**：`<img :src="src">` 可简写为 `<img :src>`
- **更快的模板解析**：解析器性能提升约 2x

## 9.3.【Vue 3.5 新特性】

- **响应式 Props 解构**：解构后的 props 保持响应性，支持原生默认值语法
- **useId() API**：为 SSR 应用生成唯一 ID
- **改进的 watch/watchEffect**：支持直接监听 props 解构值
- **更快的响应式系统**：内存占用减少，性能提升
